...
class ViewController: UIViewController {
	...
	@objc func submitTapped(sender: UIButton) {
		...
		if let solutionPositivon = solutions.firstIndex(of: answerText) {
			activatedButtons.removeAll()
			
			var splitAnswers = answersLabel.text?.components(separatedBy: "\n")
			splitAnswers?[solutionPositivon] = answerText
			...
			if score == 1 {
				...
				ac.addAction(UIAlertAction(title: "Lets go!", style: .default, handler: performSelector(inBackground: levelUp(), withObject: nil)))
			...
			let ac = UIAlertController(title: "Wrong phrase", message: "You typed an incorrect answer, try again", preferredStyle: .alert)
	...
	@objc func levelUp(action: UIAlertAction) {
		...
		performSelector(onMainThread: loadLevel(), withObject: nil, waitUntilDone: false)
